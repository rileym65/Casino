...
rcs_Display *rcs_openDisplay(const char* display_name) {
#ifdef X
...
rcs_Font rcs_openFont(rcs_Display* d,const char* font) {
#ifdef X
...
#ifdef X
void  _rcs_buildColors(rcs_Display* d,XColor* colors,const char** data,int *none) {
  XColor clr1,clr2;
#endif
#ifdef WIN32
...
void  rcs_drawSubPixmap(rcs_Display* d,rcs_Window win,rcs_GC gc,int dx,int dy,const char** data,int sx,int sy,int w,int h) {
  int        sizeX,sizeY,numColors;
  int        x,y;
  int        index;
  int        none;
