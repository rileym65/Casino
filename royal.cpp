...
#define SPCY 25
#ifdef GRAPHIC
#include "images/royal.xpm"
#endif

Royal::Royal(int decks,int jokers,float minbet):
    CardGames(decks,jokers,minbet) {
  betAmount=2;
  }

int Royal::playerX(int p)
{
#ifdef GRAPHIC
  switch (p) {
    case 0:return 9;
...
float Royal::placeBet(int player) {
  float betReturn;
  char  flag;
  char  bflag;
#ifdef GRAPHIC
    ...
    switch(event.type) {
      case EVENT_BUTTON_PRESS:
           ...
           if (event.d1 == 1 &&
             ...
             bflag |= 1;
             showPlayers();
             players[player]->paintBets();
             displayScreen();
             }
           if (event.d1 == 1 && bflag!=0 &&
               event.d2>=playerBetX(player)+50 &&
               event.d2<=playerBetX(player)+85 &&
               event.d3>=playerBetY(player)-50 &&
               event.d3<=playerBetY(player)-20 ) {
             betReturn=players[player]->placeBet(betAmount,6,playerBetX(player)+70,playerBetY(player)-35);
             bflag |= 2;
             showPlayers();
             players[player]->paintBets();
             displayScreen();
...
#ifdef GRAPHIC
  rcs_GC gc;
  table=rcs_xpmToPixmap(display,mainWindow,royal_xpm);
#endif
  bet=1;
  deck->shuffle();
  while (bet!=0) {
    ...
    for (i=0;i<5;i++) if (players[i]!=NULL) {
      if (players[i]->playerType()=='H') {
        ...
        betReturn=players[i]->placeBet(1,6,playerBetX(i)+70,playerBetY(i)-35);
#ifdef CONSOLE
        GotoXY(playerX(i),playerY(i)+6);
        sprintf(buffer,"%12.2f",players[i]->money());
        Output("Money $");
...
char Royal::computerPlayer(Hand* dHand,Hand* pHand)
{
  int count;
  char r;
  r='S';
...
void Royal::showHand(Hand *h,int hidden) {
#ifdef GRAPHIC
  rcs_GC gc;
    gc=rcs_openGC(display,mainScreen);
...
/* check royal match */
  for (i=0;i<5;i++) if (players[i] != NULL) {
    ...
    if (hand->cards[0]->getSuit() == hand->cards[1]->getSuit()) {
      royal[i]=3;
      if (hand->cards[0]->value() == 12 && hand->cards[1]->value()==13)
        royal[i]=10;
      if (hand->cards[1]->value() == 12 && hand->cards[0]->value()==13)
        royal[i]=10;
      } else royal[i]=0;
  }
      ...
      if (hand->numCards>0) {
        ...
        while (flag==' ') {
...
#ifdef GRAPHIC
            bFlags=0;
            if ((count==10 || count==11) && hand->numCards==2) bFlags |= 1;
          ...
          if (buffer[0]=='h' || buffer[0]=='H') {
              players[i]->newCard(deck->dealCard(),cHand);
              hand=players[i]->getHand(cHand);
...
/*                Output("  >>>BUST<<<");  */
    ...
    players[i]->showBets();
            ...
            if (buffer[0]=='d' || buffer[0]=='D') {
              if ((count==10 || count==11) && hand->numCards==2) {
                players[i]->newCard(deck->dealCard(),cHand);
                hand=players[i]->getHand(cHand);
              hand->cards[hand->numCards-1]->setPosition(playerX(i)+15*(hand->numCards-1),playerY(i)-160+cHand*SPCY);
#ifdef CONSOLE
  ...
  while (count>-18 && count<17) {
    dealerHand->cards[dealerHand->numCards++]=deck->dealCard();
  ...
  for (i=0;i<5;i++) if (players[i]!=NULL) {
    if (royal[i]!=0) {
      players[i]->winHandBets(6,royal[i]);
      players[i]->paintBets();
      displayScreen();
      showPlayers();
      } else {
#ifdef GRAPHIC
    gc=rcs_openGC(display,mainScreen);
    rcs_copyArea(display,table,mainScreen,gc,playerBetX(i)+50,playerBetY(i)-60,55,40,playerBetX(i)+50,playerBetY(i)-60);
    rcs_closeGC(display,gc);
#endif
      players[i]->loseHandBets(6);
      players[i]->paintBets();
      displayScreen();
      showPlayers();
      }
    for (cHand=0;cHand<4;cHand++) {
    hand=players[i]->getHand(cHand);
    if (hand->numCards>0) {
      count=abs(hand->bjValue());
#ifdef CONSOLE
...
#ifdef GRAPHIC
    gc=rcs_openGC(display,mainScreen);
    rcs_copyArea(display,table,mainScreen,gc,playerBetX(i)-50,playerBetY(i)-70,110,110,playerBetX(i)-50,playerBetY(i)-70);
    rcs_closeGC(display,gc);
    players[i]->paintBets();
    displayScreen();
    showPlayers();
...
void Royal::split(int p,int cHand)
{
  float amount;
  Hand* hand[4];
  Card* card1;
