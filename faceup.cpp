...
#define SPCY 25
#ifdef GRAPHIC
#include "images/faceup.xpm"
#endif

Faceup::Faceup(int decks,int jokers,float minbet):
    CardGames(decks,jokers,minbet) {
  betAmount=minbet;
  }

int Faceup::playerX(int p)
{
  switch (p) {
    case 0:return 9;
    case 1:return 166;
...
float Faceup::placeBet(int player) {
  float betReturn;
  char  flag;
  rcs_Event event;
  rcs_GC    gc;
...
void Faceup::play()
{
  ...
  float betReturn;
  rcs_GC gc;
  table=rcs_xpmToPixmap(display,mainWindow,faceup_xpm);
  bet=1;
  deck->shuffle();
  while (bet!=0) {
    gc=rcs_openGC(display,mainScreen);
    ...
    for (i=0;i<5;i++) if (players[i]!=NULL) {
      if (players[i]->playerType()=='H') {
        ...
        betReturn=players[i]->placeBet(minBet,0,playerBetX(i)-30,playerBetY(i)+20);
        }
      showPlayers();
      players[i]->paintBets();
      displayScreen();
...
char Faceup::computerPlayer(Hand* dHand,Hand* pHand)
{
  int count;
  char r;
  r='S';
...
void Faceup::showHand(Hand *h,int hidden) {
#ifdef GRAPHIC
  rcs_GC gc;
    gc=rcs_openGC(display,mainScreen);
...
void Faceup::split(int p,int cHand)
{
  float amount;
  Hand* hand[4];
  Card* card1;
  ...
  if (numSplits==1) return;
    ...
    switch(numSplits) {
      ...
      case 2:x=playerBetX(p)-30; y=playerBetY(p)-20; break;
      case 3:x=playerBetX(p)+20; y=playerBetY(p)-20; break;
      }
