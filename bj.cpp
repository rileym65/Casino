...
#define SPCY 25
#ifdef GRAPHIC
#include "images/bj.xpm"
#endif

Bj::Bj(int decks,int jokers,float minbet):
    CardGames(decks,jokers,minbet) {
  betAmount=2;
  }

int Bj::playerX(int p)
{
#ifdef GRAPHIC
  switch (p) {
    case 0:return 9;
...
#ifdef GRAPHIC
  rcs_GC gc;
  table=rcs_xpmToPixmap(display,mainWindow,bj_xpm);
#endif
  bet=1;
  deck->shuffle();
  while (bet!=0) {
...
char Bj::computerPlayer(Hand* dHand,Hand* pHand)
{
  int count;
  char r;
  r='S';
...
void Bj::showHand(Hand *h,int hidden) {
#ifdef GRAPHIC
  rcs_GC gc;
    gc=rcs_openGC(display,mainScreen);
