...
#define SPCY 25
#ifdef GRAPHIC
#include "images/superfun.xpm"
#endif

SuperFun::SuperFun(int decks,int jokers,float minbet):
    CardGames(decks,jokers,minbet) {
  betAmount=2;
  }

int SuperFun::playerX(int p)
{
#ifdef GRAPHIC
  switch (p) {
    case 0:return 9;
...
float SuperFun::placeBet(int player) {
  float betReturn;
  char  flag;
#ifdef GRAPHIC
  rcs_Event event;
...
#ifdef GRAPHIC
  rcs_GC gc;
  table=rcs_xpmToPixmap(display,mainWindow,superfun_xpm);
#endif
  bet=1;
  deck->shuffle();
  while (bet!=0) {
    ...
    for (i=0;i<5;i++) if (players[i]!=NULL) {
      if (players[i]->playerType()=='H') {
        ...
        betReturn=players[i]->placeBet(players[i]->getBet(),0,playerBetX(i)-30,playerBetY(i)+20);
#ifdef CONSOLE
        GotoXY(playerX(i),playerY(i)+6);
        sprintf(buffer,"%12.2f",players[i]->money());
        Output("Money $");
...
char SuperFun::computerPlayer(Hand* dHand,Hand* pHand)
{
  int count;
  char r;
  r='S';
...
void SuperFun::showHand(Hand *h,int hidden) {
#ifdef GRAPHIC
  rcs_GC gc;
    gc=rcs_openGC(display,mainScreen);
...
#endif
        bjValue=1;
        if (hand->cards[0]->getSuit()=='D' &&
            hand->cards[1]->getSuit()=='D') bjValue=2;
        players[i]->winHandBets(0,bjValue);
        players[i]->newHand(0);
        }
      else {
#ifdef CONSOLE
...
/*                Output("  >>>BUST<<<");  */
  ...
  for (i=0;i<5;i++) if (players[i]!=NULL) for (cHand=0;cHand<4;cHand++) {
    ...
    if (hand->numCards>0) {
      ...
      GotoXY(playerX(i),playerY(i)+cHand);
#endif
      if (count<21 && hand->numCards>5) {
        players[i]->winHandBets(cHand,1);
        }
      else if (count==21 && hand->numCards>4) {
...
void SuperFun::split(int p,int cHand)
{
  float amount;
  Hand* hand[4];
  Card* card1;
