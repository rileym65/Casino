...
#ifdef GRAPHIC
#include "images/carib.xpm"
#endif

Carib::Carib(int decks,int jokers,float minbet):
  Poker(decks,jokers,minbet) {
  betAmount=minbet;
  }

int Carib::playerX(int p)
{
  switch (p) {
    case 0:return 9;
    case 1:return 166;
...
void Carib::showJackpot() {
  ...
  font=rcs_openFont(display,BET_FONT);
  rcs_setFont(display,gc,font);
  sprintf(buffer,"$%12.2f",progressives[PROG_CARIB]);
...
float Carib::placeBet(int player) {
  float betReturn;
  char  flag;
  char  betFlags;
  rcs_Event event;
...
void Carib::play()
{
  ...
  float betReturn;
  rcs_GC gc;
  table=rcs_xpmToPixmap(display,mainWindow,carib_xpm);
...
void Carib::round()
{
  ...
  rcs_GC gc;
/*  C.Value=10; C.Suit='D'; */
  for (i=0;i<PLAYERS_CARIB;i++) if (players[i]!=NULL)
    players[i]->newHand(0);
  for (i=0;i<PLAYERS_CARIB;i++) if (players[i]!=NULL) {
    for (j=0;j<5;j++)  {
  ...
  for (i=0;i<PLAYERS_CARIB;i++)
    ...
    if ((high&(15l<<28))==PK_PAIR) {
      ...
      payout=7;
      if (players[i]->getHandBet(1)>0) {
          players[i]->winHandBets(1,150);
          progressives[PROG_CARIB]-=150;
        }
      }
      ...
      payout=50;
      if (players[i]->getHandBet(1)>0) {
          players[i]->winHandBets(1,progressives[PROG_CARIB]/10);
          progressives[PROG_CARIB]-=(progressives[PROG_CARIB]/10);
        }
      }
      ...
      payout=100;
      if (players[i]->getHandBet(1)>0) {
          players[i]->winHandBets(1,progressives[PROG_CARIB]);
